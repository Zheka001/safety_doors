...
def count_poi(plane1: np.ndarray, plane2: np.ndarray, other: np.ndarray, alpha=[5, 6, 7]):
    vars1 = find_var(plane1)
    vars2 = find_var(plane2)
...
def extract_features(pcd_path: str, alpha_list, voxel_size=None, show=False):
    ...
    return count_poi(plane1_points, plane2_points, outliers_points, alpha_list)

if __name__ == '__main__':
    pcd = "data/point_cloud_train/clouds_tof/cloud_0_1620666065884775.pcd"
    extract_features(pcd, [5, 6, 7, 8], voxel_size=0.3, show=True)
